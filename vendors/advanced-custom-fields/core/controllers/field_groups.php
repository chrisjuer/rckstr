...
<div id="acf-about" class="acf-content">
	...
	<div class="acf-content-title">
		...
		<h2><?php _e("Thank you for updating to the latest version!",'acf'); ?> <br /><?php printf(__('Plugins %s is more polished and enjoyable than ever before. We hope you like it.','acf'),$version); ?></h2>
	</div>
	<!-- / acf-content-title -->
...
<?php if( $tab == 'whats-new' ): 
		...
		<p><?php printf(__('All actions & filters have received a major facelift to make customizing Plugins even easier! Please %sread this guide%s to find the updated naming convention.','acf'),'<a href="http://www.advancedcustomfields.com/resources/getting-started/migrating-from-v3-to-v4/" target="_blank">','</a>'); ?></p>
		...
		<div class="acf-alert acf-alert-error">
			<p><strong><?php _e("Really Important!",'acf'); ?></strong> <?php printf(__('If you updated the Plugins plugin without prior knowledge of such changes, please roll back to the latest %sversion 3%s of this plugin.','acf'),'<a href="http://wordpress.org/extend/plugins/advanced-custom-fields/developers/">','</a>'); ?></p>
		</div>
...
<?php elseif( $tab == 'download-add-ons' ): ?>
		...
		<p><?php _e("Previously, all Add-ons were unlocked via an activation code (purchased from the Plugins Add-ons store). New to v4, all Add-ons act as separate plugins which need to be individually downloaded, installed and updated.",'acf'); ?></p>
